...
- **THEN** el sistema responde `201 Created` con el producto creado incluyendo el campo `imagen_url` (null si no se proveyó), y las relaciones en `ProductoCategoria` y `ProductoIngrediente` quedan persistidas
...
#### Scenario: Actualizar producto (otros campos)

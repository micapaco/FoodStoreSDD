  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-sm text-gray-500">Detalle del pedido</p>
          <h1 className="text-2xl font-bold text-gray-900">
            {pedidoId ? `Pedido #${pedidoId}` : 'Pedido invalido'}
          </h1>
        </div>
        <Link to="/pedidos" className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-800">
          Volver
        </Link>
      </div>
        ...
        <>
          <section className="mt-6 grid gap-4 md:grid-cols-3">
            <div className="rounded-lg border border-gray-200 bg-white p-5">
              ...
              <p className="mt-2 text-lg font-bold text-gray-900">{data.estadoCodigo}</p>
              <p className="mt-1 text-sm text-gray-500">{formatDate(data.createdAt)}</p>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-5">
              ...
              <p className="mt-1 text-sm text-gray-500">{data.pago?.statusDetail ?? data.formaPagoCodigo}</p>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-5">
              ...
              <div className="rounded-lg border border-gray-200 bg-white p-5">
                ...
                {data.direccionSnapshot ? (
                  <div className="mt-3 text-sm text-gray-700">
                    <p className="font-semibold text-gray-900">{data.direccionSnapshot.alias ?? 'Direccion registrada'}</p>
                  ...
                  <p className="mt-3 text-sm text-gray-600">Retiro en local.</p>
                )}
              </div>
                ...
                <ol className="mt-4 space-y-4">
                  {data.historial.map((entry) => (
                    <li key={entry.id} className="border-l-2 border-orange-200 pl-4">
                      ...
                      {entry.motivo && <p className="mt-1 text-sm text-gray-600">{entry.motivo}</p>}
                    </li>
                  ))}
                </ol>
              </div>
